...
alter table public.app_user_roles
  add column if not exists access_scopes text[] not null default '{}'::text[];

update public.app_user_roles
set access_scopes = '{}'::text[]
where access_scopes is null;

update public.app_user_roles
set
  role = 'member',
  access_scopes = case
    when 'students' = any(coalesce(access_scopes, '{}'::text[]))
      then access_scopes
    else array['students']::text[]
  end
where role in ('students_only', 'teacher');
...
create index if not exists idx_app_user_roles_access_scopes on public.app_user_roles using gin (access_scopes);
...
create index if not exists idx_tasks_event_date on public.tasks(event_date);
...
create policy students_access_scoped
on public.students
for all
to authenticated
using (
  exists (
    ...
    where aur.user_id = auth.uid()
      and (
        aur.role = 'admin'
        or 'students' = any(coalesce(aur.access_scopes, '{}'::text[]))
      )
  )
)
with check (
...
create policy finances_access_scoped
on public.finances
for all
to authenticated
using (
...
create policy attendance_access_scoped
...
create policy tasks_read_owned
on public.tasks
for select
to authenticated
using (
...
begin
  ...
  select
    ...
    coalesce(aur.access_scopes, '{}'::text[])::text[] as access_scopes,
...
drop function if exists public.upsert_user_role(uuid, text, text[], text[]);
create or replace function public.upsert_user_role(
  p_user_id uuid,
  p_role text,
  p_access_scopes text[] default '{}'::text[],
...
begin
  ...
  if p_role not in ('admin', 'member') then
    raise exception 'Invalid role. Allowed values: admin, member.';
  end if;
  ...
  if p_role = 'member' then
    select coalesce(array_agg(distinct trimmed_scope), '{}'::text[])
    into normalized_access_scopes
    from (
      select btrim(raw_scope) as trimmed_scope
      from unnest(coalesce(p_access_scopes, '{}'::text[])) as raw_scope
    ) as normalized_scopes
    where trimmed_scope in ('students', 'tasks');

    if coalesce(array_length(normalized_access_scopes, 1), 0) = 0 then
      raise exception 'At least one access scope is required for member access.';
    end if;
  end if;

  if p_role = 'member' and 'students' = any(coalesce(normalized_access_scopes, '{}'::text[])) then
    ...
    if coalesce(array_length(normalized_assigned_teachers, 1), 0) = 0 then
      raise exception 'At least one assigned teacher is required for Student Access.';
    end if;
  end if;

  insert into public.app_user_roles (user_id, role, access_scopes, assigned_teachers)
  values (
    p_user_id,
    p_role,
    case when p_role = 'member' then normalized_access_scopes else '{}'::text[] end,
    case
      when p_role = 'member' and 'students' = any(coalesce(normalized_access_scopes, '{}'::text[]))
        then normalized_assigned_teachers
      else '{}'::text[]
    end
  )
  on conflict (user_id)
...
revoke all on function public.upsert_user_role(uuid, text, text[], text[]) from public;
grant execute on function public.upsert_user_role(uuid, text, text[], text[]) to authenticated;

-- Example role assignment (run with SQL editor/service role):
-- insert into public.app_user_roles (user_id, role, access_scopes, assigned_teachers)
-- values ('<auth_user_uuid>', 'member', array['students', 'tasks'], array['Neha Verma', 'Rahul Mehta'])
-- on conflict (user_id)
-- do update set
--   role = excluded.role,
--   access_scopes = excluded.access_scopes,
